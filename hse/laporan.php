...
<div class="fade-in-up">
    ...
    <div id="laporanContainer">
        ...
        <div style="text-align:center; padding:20px 0; border-bottom:2px solid #1a1a2e; margin-bottom:20px;">
            <h1 style="font-weight:800; color:#1a1a2e; margin:0;">ERGO<span style="color:#4fc3f7;">CARE</span></h1>
            <p style="color:#64748b; margin:5px 0;">Sistem Assessment Ergonomi &amp; Psikososial</p>
            <p style="color:#475569; font-weight:600; margin:5px 0;">
                LAPORAN ASSESSMENT
            </p>
            ...
            if (!empty($filter_info)) {
                echo '<p style="color:#475569; font-size:13px;"><strong>Filter:</strong> ' . implode(' | ', $filter_info) . '</p>';
            }
            ?>
            <p style="color:#475569; font-size:13px;"><strong>Total Data:</strong> <?= $total_data ?> karyawan</p>
        </div>
            ...
            <table style="width:100%; border-collapse:collapse; font-size:13px;">
                <thead>
                    <tr style="background:#1a1a2e; color:white;">
                        <th style="padding:10px 8px; text-align:center;">No</th>
                        <th style="padding:10px 8px; text-align:left;">Nama</th>
                        <th style="padding:10px 8px; text-align:left;">Kode</th>
                        <th style="padding:10px 8px; text-align:left;">Departemen</th>
                        <th style="padding:10px 8px; text-align:left;">Periode</th>
                        <th style="padding:10px 8px; text-align:center;">Skor Ergo</th>
                        <th style="padding:10px 8px; text-align:center;">Skor Psiko</th>
                        <th style="padding:10px 8px; text-align:center;">Risiko Ergo</th>
                        <th style="padding:10px 8px; text-align:center;">Risiko Psiko</th>
                        <th style="padding:10px 8px; text-align:center;">Tanggal</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    ...
                    <tr style="border-bottom:1px solid #e2e8f0;">
                        ...
                        <td style="padding:8px;"><strong><?= htmlspecialchars($row['full_name']) ?></strong></td>
                        <td style="padding:8px;"><?= $row['employee_code'] ?></td>
                        <td style="padding:8px;"><?= $row['department_name'] ?></td>
                        <td style="padding:8px;"><?= $row['period_name'] ?></td>
                        <td style="padding:8px; text-align:center;"><?= $row['ergonomic_score'] ?>/30</td>
                        <td style="padding:8px; text-align:center;"><?= $row['psychosocial_score'] ?>/25</td>
                        <td style="padding:8px; text-align:center;">
                            <span style="background:<?= $ergo_color ?>20; color:<?= $ergo_color ?>; padding:2px 12px; border-radius:12px; font-weight:600; font-size:12px;">
                                <?= $row['ergo_risk'] ?>
                            </span>
                        </td>
                        <td style="padding:8px; text-align:center;">
                            <span style="background:<?= $psiko_color ?>20; color:<?= $psiko_color ?>; padding:2px 12px; border-radius:12px; font-weight:600; font-size:12px;">
                                <?= $row['psiko_risk'] ?>
                            </span>
                        </td>
                        <td style="padding:8px; text-align:center; font-size:12px; color:#64748b;">
        ...
        <div style="margin-top:30px; text-align:center; color:#94a3b8; font-size:11px; border-top:1px solid #e2e8f0; padding-top:15px;">
            <p>Dicetak dari ERGOCARE - Sistem Assessment Ergonomi &amp; Psikososial</p>
            <p>&copy; <?= date('Y') ?> PT Radiant Utama</p>
        </div>
    </div>
</div>
